...
typedef struct ReorderBufferIterTXNState
{
	binaryheap *heap;
	Size nr_txns;
	ReorderBufferChange *old_change;
...
ReorderBufferIterTXNNext(ReorderBuffer *cache, ReorderBufferIterTXNState *state)
{
	...
	if (!dlist_is_empty(&entry->txn->subtxns))
		elog(LOG, "tx with subtxn");

	/* free memory we might have "leaked" in the last *Next call */
	if (state->old_change != NULL)
	{
		ReorderBufferReturnChange(cache, state->old_change);
		state->old_change = NULL;
	}
